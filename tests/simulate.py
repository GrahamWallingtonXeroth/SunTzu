...
from map_gen import BOARD_SIZE, get_hex_neighbors, hex_distance
...
def _visible_enemies(force: Force, player_id: str, game_state: GameState, max_range: int = 2) -> list[Force]:
    ...
    return [
        ...
        if hex_distance(force.position[0], force.position[1], f.position[0], f.position[1]) <= max_range
    ]
...
def _move_toward(force: Force, target: tuple[int, int], game_state: GameState) -> tuple[int, int] | None:
    ...
    def plan(self, player_id: str, game_state: GameState, rng: random.Random) -> list[Order]:
        ...
        for force in alive:
            ...
            if sovereign_pos and force.power and force.power >= 4:
                dist = hex_distance(force.position[0], force.position[1], sovereign_pos[0], sovereign_pos[1])
            ...
            if force.power and force.power >= 4 and enemies:
                ...
                dist_e = hex_distance(
                    force.position[0], force.position[1], target_enemy.position[0], target_enemy.position[1]
                )
        ...
        for force in player.get_alive_forces():
            ...
            if sovereign_pos and force.power and force.power >= 4:
                ...
                if moves:
                    ...
                    best = min(pool, key=lambda m: hex_distance(m[0], m[1], sovereign_pos[0], sovereign_pos[1]))
                    orders.append(Order(OrderType.MOVE, force, target_hex=best))
                    continue
            ...
            for enemy in enemies_near:
                if enemy.id in player.known_enemy_powers:
                    ...
                    if force.power and force.power >= known_power + 2:
                        dist = hex_distance(force.position[0], force.position[1], enemy.position[0], enemy.position[1])
                        if dist <= 2 and _can_order(force, player, OrderType.CHARGE):
                            orders.append(Order(OrderType.CHARGE, force, target_hex=enemy.position))
                        elif dist <= 1:
                            orders.append(Order(OrderType.MOVE, force, target_hex=enemy.position))
            ...
            for enemy in enemies_adjacent:
                if enemy.id in player.known_enemy_powers:
                    ...
                    if force.power and known_power >= force.power + 1:
                        ...
                        if moves:
                            best = max(
                                moves, key=lambda m: hex_distance(m[0], m[1], enemy.position[0], enemy.position[1])
                            )
                            orders.append(Order(OrderType.MOVE, force, target_hex=best))
                            retreated = True
                            break
            ...
            if on_contentious:
                ...
                if (
                    ...
                    and force.power
                    and force.power >= 3
                    and _can_order(force, player, OrderType.AMBUSH)
                ):
...
class AmbushStrategy(Strategy):
    ...
    def plan(self, player_id: str, game_state: GameState, rng: random.Random) -> list[Order]:
        ...
        for force in player.get_alive_forces():
            ...
            if on_contentious and not enemies_near and _can_order(force, player, OrderType.FORTIFY):
                orders.append(Order(OrderType.FORTIFY, force))
                continue
...
class CoordinatorStrategy(Strategy):
    ...
    def plan(self, player_id: str, game_state: GameState, rng: random.Random) -> list[Order]:
        ...
        for force in alive:
            ...
            adj_friends = sum(1 for f in alive if f.id != force.id and is_adjacent(f.position, force.position))
            on_contentious = any(force.position == c for c in contentious)
                    ...
                    if moves:
                        ...
                        for m in moves:
                            ...
                            adj = sum(
                                1
                                for f in alive
                                if f.id != force.id and hex_distance(f.position[0], f.position[1], m[0], m[1]) <= 1
                            )
...
class DominationStallerStrategy(Strategy):
    ...
    def plan(self, player_id: str, game_state: GameState, rng: random.Random) -> list[Order]:
        ...
        for force in player.get_alive_forces():
            enemies_adjacent = [
                e
                for e in _visible_enemies(force, player_id, game_state, max_range=1)
                if hex_distance(force.position[0], force.position[1], e.position[0], e.position[1]) <= 1
            ]
            on_contentious = any(force.position == c for c in contentious)
            ...
            uncontrolled = [
                ...
                if game_state.get_force_at_position(c) is None
                or game_state.get_force_owner(game_state.get_force_at_position(c).id).id != player_id
            ]
            if uncontrolled:
                target = min(uncontrolled, key=lambda c: hex_distance(force.position[0], force.position[1], c[0], c[1]))
            else:
...
def run_game(
    ...
    record = GameRecord(
        ...
        victory_type=None,
        turns=0,
        p1_strategy=p1_strategy.name,
        p2_strategy=p2_strategy.name,
    ...
    while game.phase != "ended" and game.turn <= MAX_TURNS:
        ...
        for pid in ["p1", "p2"]:
            ...
            for f in alive:
                ...
                if not has_supply(f, p.forces, supply_cfg["supply_range"], max_hops=supply_cfg["max_supply_hops"]):
        ...
        for order_list, _pid in [(p1_orders, "p1"), (p2_orders, "p2")]:
            for o in order_list:
                pw = o.force.power
                if pw is not None:
        ...
        for combat_result in result.get("combats", []):
            ...
            for role, _pid_key in [("attacker", "attacker_id"), ("defender", "defender_id")]:
        ...
        for scout_result in result.get("scouts", []):
            ...
            if (
                not is_sov_reveal
                and scout_result.get("scout_type") == "band"
                and scout_result.get("revealed_band") == "band_low"
                and scout_result.get("actual_power") == 1
            ):
                is_sov_reveal = True
            if is_sov_reveal:
        ...
        for pid in ["p1", "p2"]:
            p = game.get_player_by_id(pid)
            if p:
                record.domination_turns_reached[pid] = max(record.domination_turns_reached[pid], p.domination_turns)
        ...
        for pid in ["p1", "p2"]:
            ...
            if p:
                ...
                if controlled:
                    record.contentious_control_turns[pid] = record.contentious_control_turns.get(pid, 0) + 1
        ...
        snap = {
            ...
            "p1_killed_this_turn": 0,
            "p2_killed_this_turn": 0,
            "p1_hidden_to_opp": 0,
            "p2_hidden_to_opp": 0,
        ...
        for pid in ["p1", "p2"]:
            ...
            if opp:
                hidden = sum(1 for f in alive if not f.revealed and f.id not in opp.known_enemy_powers)
