...
def score_arc_length(records: list[GameRecord]) -> tuple[float, str]:
    ...
    detail = f"avg_turns={avg:.1f}, midgame_reach={midgame_pct:.1%}, lategame_reach={lategame_pct:.1%}"
...
def score_lead_changes(records: list[GameRecord]) -> tuple[float, str]:
    ...
    detail = f"avg_lead_changes={avg_changes:.2f}, games_with_changes={pct_with_change:.1%}"
    return _clamp(score), detail
...
def score_decisive_moments(records: list[GameRecord]) -> tuple[float, str]:
    ...
    for r in records:
        ...
        turns_with_kills = sum(
            1 for s in r.turn_snapshots if s.get("p1_killed_this_turn", 0) + s.get("p2_killed_this_turn", 0) > 0
        )
    ...
    total_kills_per_game = [(r.p1_forces_lost + r.p2_forces_lost) for r in records]
    avg_kills = sum(total_kills_per_game) / len(total_kills_per_game)
    avg_spread * sum(r.turns for r in records) / len(records)
    ...
    multi_crisis = sum(
        ...
        for r in records
        if r.turn_snapshots
        and sum(1 for s in r.turn_snapshots if s.get("p1_killed_this_turn", 0) + s.get("p2_killed_this_turn", 0) > 0)
        >= 2
    ) / max(len(records), 1)
    crisis_score = min(multi_crisis / 0.40 * 10.0, 10.0)

    score = spread_score * 0.6 + crisis_score * 0.4
    detail = f"kill_spread={avg_spread:.1%}, avg_kills={avg_kills:.1f}/game, multi_crisis_games={multi_crisis:.1%}"
    return _clamp(score), detail
...
def score_story_diversity(records: list[GameRecord]) -> tuple[float, str]:
    ...
    detail = f"entropy={entropy:.2f}/{max_entropy:.2f} bits, active_types={active_types}, dist=[{dist_str}]"
    return _clamp(score), detail
...
def score_comeback_viability(records: list[GameRecord]) -> tuple[float, str]:
    ...
    detail = f"comeback_rate={comeback_rate:.1%} ({behind_wins}/{behind_total} games with clear leader at midpoint)"
...
def score_phase_transitions(records: list[GameRecord]) -> tuple[float, str]:
    ...
    detail = f"JS_divergence={js_div:.3f}, long_games={len(long_games)}, early=[{early_str}], late=[{late_str}]"
...
def score_info_action_coupling(records: list[GameRecord]) -> tuple[float, str]:
    ...
    detail = f"scout_win_rate={scout_advantage:.1%} vs no_scout, gap={gap:.1%}"
...
def score_outcome_uncertainty(records: list[GameRecord]) -> tuple[float, str]:
    ...
    for key, games in matchups.items():
        ...
        s1_wins = sum(
            1
            for g in decided
            if (g.winner == "p1" and g.p1_strategy == s1) or (g.winner == "p2" and g.p2_strategy == s1)
        )
    ...
    detail = f"avg_predictability={avg_pred:.1%}, avg_upset_rate={avg_upset:.1%}, matchups={len(predictabilities)}"
...
def score_counter_strategy(records: list[GameRecord]) -> tuple[float, str]:
    ...
    variances = []
    for _strat, rates in strategy_matchup_rates.items():
        if len(rates) >= 3:
...
def compute_narrative_scores(verbose=True):
    ...
    _all_records, competitive = _run_competitive_tournament()
...
def test_narrative_score():
    ...
    _scores, overall, _narrative, _calc = compute_narrative_scores(verbose=True)
...
def test_anti_calculability_floor():
    ...
    _scores, _, _, calc = compute_narrative_scores(verbose=False)
    assert calc >= 5.0, (
