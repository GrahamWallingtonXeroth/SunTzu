"""Tests for combat resolution: power values + variance + ambush + charge + support + retreat."""

import random

from models import Force, Hex, Player
from resolution import calculate_effective_power, resolve_combat
...
class TestSupportBonus:
    def test_support_bonus_from_adjacent_friendly(self):
        ...
        power = calculate_effective_power(f1, game, apply_variance=False, friendly_forces=friendly)
    ...
    def test_no_support_when_no_friendly_forces_arg(self):
        ...
        game.players[0].add_force(f1)
        game.players[0].add_force(f2)
        power = calculate_effective_power(f1, game, apply_variance=False)
