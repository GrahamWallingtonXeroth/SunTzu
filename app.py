...
from state import GameState, apply_deployment, get_player_view, initialize_game
...
def new_game():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to create game: {e!s}"}), 500
...
def get_game_state(game_id: str):
    ...
    try:
        ...
        return jsonify({"error": f"Failed to get state: {e!s}"}), 500
...
def submit_action(game_id: str):
    ...
    try:
        ...
        both_submitted = all(game_state.orders_submitted.get(p.id, False) for p in game_state.players)
        ...
        view["resolve_result"] = {
            ...
            "scouts": [s for s in resolve_result.get("scouts", []) if s["player"] == player_id],
        ...
        return jsonify({"error": f"Action failed: {e!s}"}), 500
...
def concede_game(game_id: str):
    ...
    try:
        ...
        return jsonify({"error": f"Concession failed: {e!s}"}), 500
