...
def verify_fog_of_war(view: dict, full_state: GameState, player_id: str, visibility_range: int = 2) -> list[str]:
    ...
    for enemy_force in opponent.get_alive_forces():
        is_visible = any(
            hex_distance(own.position[0], own.position[1], enemy_force.position[0], enemy_force.position[1])
            <= visibility_range
    ...
    for ef in view.get("enemy_forces", []):
        ...
        if "power" in ef and ef["power"] is not None:
            ...
            if actual_force and not actual_force.revealed and force_id not in player.known_enemy_powers:
                violations.append(
                    f"LEAK: Enemy force {force_id} has power {ef['power']} in view but is neither revealed nor scouted"
                )
    ...
    for own_force in player.get_alive_forces():
        if own_force.id not in own_ids_in_view:
            violations.append(f"OMISSION: Own force {own_force.id} is alive but not in view")

    # Check: own force powers are present
    for f in view.get("your_forces", []):
        if f.get("power") is None:
            violations.append(f"OMISSION: Own force {f['id']} has no power value in view")
...
def verify_prompt_integrity(
    ...
    for enemy_force in opponent.get_alive_forces():
        if enemy_force.id not in visible_enemy_ids and enemy_force.id in prompt:
            violations.append(f"LEAK: Invisible enemy force ID '{enemy_force.id}' found in prompt")

    # 2. Check no hidden power values appear for unrevealed enemy forces
    for enemy_force in opponent.get_alive_forces():
        if (
            not enemy_force.revealed
            and enemy_force.id not in player.known_enemy_powers
            and enemy_force.id in prompt
            and enemy_force.power is not None
        ):
    ...
    for own_force in player.get_alive_forces():
        if own_force.id not in prompt:
            violations.append(f"OMISSION: Own force {own_force.id} not found in prompt")
    ...
    if f"turn {turn_str}" not in prompt.lower() and f"turn {turn_str}" not in prompt:
        ...
        for pattern in [
            ...
            if pattern in prompt:
                ...
                break
        if not found:
            violations.append(f"OMISSION: Turn number {turn} not found in prompt")
